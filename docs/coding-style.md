...
- 封装优先看功能边界，不看代码行数或实现复杂度。一个能力如果可以明确说明“接收什么、保证什么、返回什么”，内部多个步骤共同服务同一个结果，并且调用方不需要理解内部过程，就适合提取成函数或模块。
- 是否复用不是封装的必要条件。当前只调用一次的独立能力也可以封装；重复使用只是额外理由。反过来，代码很长或很复杂本身也不是拆函数的理由，没有清楚功能边界时不要仅为缩短主流程而切块。
- 只有一两行简单动作、紧邻使用、没有独立输入输出契约，或者只是把原代码换一个函数名再调用的逻辑，默认就地表达。例如只包装一次 `click()`、`get_text()` 或简单字段转换，通常没有封装价值。
...
判断是否值得封装时，重点看调用方能不能把它当成一个完整能力使用，而不是看内部有多少行代码。好的封装会隐藏实现细节，并向外提供清楚的结果或后置条件。
...
这种封装的价值来自职责完整、输入输出清楚、内部细节可以被忽略，而不是因为内部代码“够长”或“够复杂”。同理，一个很复杂但没有独立职责、只是主业务连续步骤的一部分，也不应为了降低单个函数长度而强行拆开。
...
- 拿掉函数名后，如果只剩一两行显而易见的简单调用，通常不需要再包一层。
