...
- **默认先不拆。** 只调用一次、只服务当前业务流程的读取、校验、判断、计算、写回等步骤，默认留在调用处。即使这段逻辑有十几行或几十行，只要它仍是当前主流程的一部分，就不要仅为了缩短 `main()`、给代码起名字、分层或区分“数据层 / 业务层”而提取函数。
...
这种封装的价值来自它向调用方提供了一个完整后置条件——“拿到可直接工作的页面”——并隐藏登录恢复、页面就绪等实现细节，而不是因为内部代码“够长”或“够复杂”。同理，一个很复杂但只是主业务连续步骤的一部分，也不应为了降低单个函数长度而强行拆开。
