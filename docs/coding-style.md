...
- **默认先不拆。** 只调用一次、只服务当前业务流程的简单读取、校验、判断、计算、写回等步骤，默认留在调用处。不要仅为了缩短当前函数、给代码起名字、分层或区分“数据层 / 业务层”而提取函数。
- “职责单一”“有输入输出”“可以叫作某种能力”都不足以证明值得封装。像 `read_payment_row()` 这类“读一行再组字典”、`_validate_payment()` 这类只把当前流程的一段校验搬走的函数，如果只在紧邻流程中调用一次，通常应直接内联。
- 当一段逻辑本身构成完整业务阶段，提取后能让调用方直接看到更高层的业务顺序，并且调用方不需要理解该阶段内部步骤时，可以提取为中层函数。是否提取看业务边界，不看代码行数或调用次数。提取后不要继续把这个完整阶段拆成多个只包装简单读取、判断、转换或写回的小函数。
- 单次调用不是绝对禁止封装。只有一个能力本身可以被调用方当成完整结果使用，调用方无需理解内部执行顺序，并且函数确实隐藏了一组共同保证明确结果的实现细节时，才值得提取。是否复用只是额外理由，不是前提。
- 只包一两个简单调用、简单字段转换，或者只是把原代码换一个函数名再调用的逻辑，默认就地表达。例如只包装一次 `click()`、`get_text()` 通常没有封装价值。
...
```
- 是否使用类同样按对象边界判断，不做“普通业务默认不用类”的全局限制。多个操作天然围绕同一对象、需要共享必要状态或生命周期，并且类能给调用方提供稳定结果接口时，可以使用；如果只是把几个无状态函数装进类里分组，则没有必要。
...
代码分层的目标不是增加函数数量，而是降低调用方需要同时理解的信息量。

上一层只保留当前业务阶段需要理解的输入、结果和控制流程；仅服务于当前实现过程的字段、索引、中间状态和解析细节，应留在当前层内部，不无意义地继续向上传递。

函数参数、返回值和共享状态也遵循这一原则：只暴露调用方后续确实需要使用或判断的数据，不因为“可能有用”而扩大接口。
...
商品状态匹配、字段兼容、SKU 解析、销量字段读取等细节留在“解析当前页”内部。反过来，也不要为了让上一层更短，把一个完整业务阶段继续拆成多个只包装简单读取、判断、转换或写回的小函数。

### 真正独立能力的例外

默认不拆当前业务流程后，再判断是否存在真正值得提取的独立能力。重点看调用方能不能只关心结果、完全忽略内部步骤，而不是看内部有多少行代码。

例如 `_get_page(url)` 可以是一个合理的独立能力：内部负责根据 URL 获取或复用页面、检查并恢复登录状态、确认业务依赖的 JavaScript 已加载完成，最后返回一个可直接用于后续业务的 `page`。调用方只关心“拿到一个已经可以工作的页面”，不需要知道登录和页面就绪检查具体怎么做。
...
这种封装的价值来自它向调用方提供了一个完整后置条件——“拿到可直接工作的页面”——并隐藏登录恢复、页面就绪等实现细节，而不是因为内部代码“够长”或“够复杂”。同理，一段逻辑如果不构成完整业务阶段，也不应仅为了降低单个函数长度而强行拆开。

简单判断：

- 能否给这个能力写出一句稳定、具体的职责描述，而不是“处理一下”“辅助一下”。
- 调用方是否只需要输入和结果，而不需要知道内部执行顺序。
- 内部步骤是否共同保证同一个结果或后置条件。
- 它是否真的隐藏了调用方不需要关心的细节；如果调用方仍要理解这些步骤，拆出去通常只是增加跳转。
- 拿掉函数名后，如果只是当前流程的一小段直接逻辑，尤其是读数据、校验、判断、转换或写回，通常不需要再包一层。

## 5. 直白与可读

- 变量名直白表达业务含义，避免 `x`、`tmp`、`obj` 等无语义名称。
- 新增函数或方法时，在函数体开头添加简短的 Sphinx / reStructuredText 文档注释。第一句直接说明这个函数“做什么”；有参数时使用 `:param 参数名: 含义`，有明确返回值时使用 `:return:` 说明返回结果。文档注释只描述职责、参数和返回结果，不展开内部执行步骤；未修改的历史函数不要求为了补文档批量整改。
...
日志首先服务于运行过程还原和故障定位，而不是追求数量。一次任务执行结束后，仅查看日志应当基本能够回答：当前执行到了哪个主要业务阶段、正在处理哪个平台 / 店铺 / 业务对象、对象发生了什么、为什么继续 / 停止 / 跳过 / 重试、程序采取了什么恢复动作、最终业务结果是什么，以及出错时失败在哪个步骤。
...
- 同一业务持续输出多条日志时，推荐保持稳定、易扫读的上下文顺序，例如 `【业务】平台｜业务对象：发生了什么`；系统级日志可使用 `【系统】模块：发生了什么`。具体标点和字段可按项目调整，但同类日志的业务、平台、对象和事件顺序应尽量一致，方便连续阅读和检索。
- 会改变程序执行路径的判断具有独立排查价值，应记录决定及原因。不要只记录“停止”“跳过”“重试”，而要让日志能够说明为什么作出该决定。
- 自动恢复、重试、重建、降级等异常路径，日志不仅记录失败原因，还应说明程序随后采取的恢复动作。例如页面失效后准备重新建页、第一次执行失败后准备完整重试。避免把“失败”和“正在恢复”拆成缺少上下文、彼此难以关联的模糊日志。
- 对定时轮询、延迟重试、周期任务等长期运行流程，如果本次业务结果会决定下一次执行时间，结果日志应同时说明下一步动作或等待时长。例如“本轮不处理，10 分钟后再次检查”，使日志能够解释程序当前为什么没有继续操作。
...
- 业务异常和异常通知属于两个不同结果，应能够分别追踪。发生异常后，如项目存在通知机制，日志应能够区分未达到通知条件、被限频抑制、通知已发送、通知发送失败等状态；通知自身失败不得覆盖或模糊原始业务异常。
...
【弹卡】抖音｜直播间A：页面激活失败，清理残留页面后重新建页；原因=页面对象已失效
【福袋发布】抖音｜直播间A：已有活动进行中，本轮不发布；10 分钟后再次检查
【弹卡】抖音｜直播间A：异常仍在持续，但 1 小时内已发送过异常通知，本次不重复发送
```

以上具体平台、字段、时间、阈值和恢复动作只用于展示推荐的日志形态，不构成通用业务规则。实际项目中的刷新周期、重试次数、通知阈值、限频时间和页面恢复策略仍由各项目自己的业务规则决定。

#### 日志对象的使用边界

当业务代码持续重复平台、店铺、任务等稳定上下文，商品 ID / SKU / 订单号等对象字段格式，日志级别、字段顺序、分隔格式或 traceback 拼接时，可以主动提取日志格式函数或专门日志对象。日志封装有实际价值时直接使用，不需要为了保持业务代码表面上的“直接”而重复拼接日志字符串。
...
原则：**业务流程保持直接，独立能力按功能边界封装；日志基础设施按需要封装；业务过程显式，日志格式隐藏；日志对象负责怎么记录，业务代码负责怎么执行。**
